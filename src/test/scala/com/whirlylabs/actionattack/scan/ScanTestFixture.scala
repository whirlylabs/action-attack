...
import com.whirlylabs.actionattack.scan.yaml.{GitHubActionsWorkflow, YamlScanner, yamlToJson}
...
trait ScanTestFixture extends AnyWordSpec
  with Matchers
...
trait YamlScanTestFixture(scansToRun: List[YamlScanner] = Nil) extends ScanTestFixture {
  ...
  def workflow(code: String): List[Finding] = {
    yamlToJson(code) match {
      ...
      case Success(workflow) => workflow.runScans(scansToRun)
    }
  }
