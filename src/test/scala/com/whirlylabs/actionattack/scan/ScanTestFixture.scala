...
import com.whirlylabs.actionattack.scan.yaml.{GitHubActionsWorkflow, YamlScanner, YamlTransformer, runScans, yamlToJson}
...
trait YamlScanTestFixture(transformersToRun: List[YamlTransformer] = Nil, scansToRun: List[YamlScanner] = Nil)
  ...
  def workflow(code: String): List[Finding] = {
    yamlToJson(code) match {
      ...
      case Success(workflow)  => runScans(workflow, transformersToRun, scansToRun)
    }
  }
