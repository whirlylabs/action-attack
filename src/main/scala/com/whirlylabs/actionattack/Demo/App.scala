package com.whirlylabs.actionattack.Demo
...
case class StatefulList[T](state: ListWidget.State, items: mutable.ArrayDeque[T]) {
...
case class Signals(sin1: Signal[Point], sin2: Signal[Point], var window: Point) {
  def on_tick(): Unit = {
    this.sin1.on_tick()
    this.sin2.on_tick()
...
case class Server(name: String, location: String, coords: Point, status: String)
...
object App {
  def apply(title: String, enhanced_graphics: Boolean): App = {
    ...
    new App(
      ...
      show_chart = true,
      progress = 0.0,
      sparkline = Signal(source = rand_signal, points = sparkline_points, tick_rate = 1),
      tasks = StatefulList.with_items(TASKS),
      logs = StatefulList.with_items(LOGS),
      signals = Signals(
        sin1 = Signal(source = sin_signal, points = sin1_points, tick_rate = 5),
        sin2 = Signal(source = sin_signal2, points = sin2_points, tick_rate = 10),
        window = Point(0.0, 20.0)
      ),
      barchart = mutable.ArrayDeque.from(EVENTS),
      servers = Array(
        Server(name = "NorthAmerica-1", location = "New York City", coords = Point(40.71, -74.00), status = "Up"),
        Server(name = "Europe-1", location = "Paris", coords = Point(48.85, 2.35), status = "Failure"),
        Server(name = "SouthAmerica-1", location = "São Paulo", coords = Point(-23.54, -46.62), status = "Up"),
        Server(name = "Asia-1", location = "Singapore", coords = Point(1.35, 103.86), status = "Up")
      ),
      enhanced_graphics
    )
  }
