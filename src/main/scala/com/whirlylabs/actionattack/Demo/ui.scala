...
import com.whirlylabs.actionattack.Demo.App
import tui.*
import tui.widgets.*
...
object ui {
  ...
  def draw_gauges(f: Frame, app: App, area: Rect): Unit = {
    val chunks =
      Layout(constraints = Array(Constraint.Length(2), Constraint.Length(3), Constraint.Length(1)), margin = Margin(1))
        .split(area)
  ...
  def draw_text(f: Frame, area: Rect): Unit = {
    val text = Text.fromSpans(
      ...
      Spans.nostyle("One more thing is that it should display unicode characters: 10€")
    )
    val block = {
  ...
  def draw_second_tab(f: Frame, app: App, area: Rect): Unit = {
    ...
    val chunks = Layout(
      constraints = Array(Constraint.Percentage(30), Constraint.Percentage(70)),
      direction = Direction.Horizontal
    ).split(area)
    ...
    val table = TableWidget(
      ...
      header = Some(
        TableWidget.Row(
          cells = Array("Server", "Location", "Status").map(cell),
          style = Style.DEFAULT.fg(Color.Yellow),
          bottomMargin = 1
        )
      ),
  ...
  def draw_third_tab(f: Frame, area: Rect): Unit = {
    val chunks =
      Layout(direction = Direction.Horizontal, constraints = Array(Constraint.Ratio(1, 2), Constraint.Ratio(1, 2)))
        .split(area)
    val colors = Array(
      Color.Reset,
      Color.Black,
